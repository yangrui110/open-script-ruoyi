<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="gameList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="游戏名称" align="center" prop="title" />
...
<script>
import { listGame, getGame, delGame, addGame, updateGame } from "@/api/script/game"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除游戏编号为"' + ids + '"的数据项？').then(function() {
        return delGame(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('script/game/export', {
        ...this.queryParams
